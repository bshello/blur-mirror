...
import com.blur.chat.config.ChatUtils;
...
public class ChatRedisCacheService {
	...
	public void addChat(ChatMessageSaveDto chatMessageSaveDto) {
		...
		redisTemplate.opsForZSet().add(NEW_CHAT, savedData,
				chatUtils.changeLocalDateTimeToDouble(savedData.getCreatedAt()));
		redisTemplate.opsForZSet().add(CHAT_SORTED_SET_ + savedData.getRoomId(), savedData,
				chatUtils.changeLocalDateTimeToDouble(savedData.getCreatedAt()));
	}

	// chat_data 조회
	public ResponseDto<List<ChatPagingResponseDto>> getChatsFromRedis(Long roomId, ChatPagingDto chatPagingDto) {

		// 마지막 채팅을 기준으로 redis의 Sorted set에 몇번째 항목인지 파악
		ChatMessageSaveDto cursorDto = ChatMessageSaveDto.builder().roomId(roomId.toString())
				.createdAt(chatPagingDto.getCursor()).message(chatPagingDto.getMessage())
				.nickname(chatPagingDto.getNickname()).build();
		...
		Set<ChatMessageSaveDto> chatMessageSaveDtoSet = zSetOperations.reverseRange(CHAT_SORTED_SET_ + roomId, rank,
				rank + 10);

		List<ChatPagingResponseDto> chatMessageDtoList = chatMessageSaveDtoSet.stream()
				.map(ChatPagingResponseDto::byChatMessageDto).collect(Collectors.toList());
	...
	public void cachingDBDataToRedis(Chat chat) {
		...
		redisTemplate.opsForZSet().add(CHAT_SORTED_SET_ + chatMessageSaveDto.getRoomId(), chatMessageSaveDto,
	...
	private void findOtherChatDataInMysql(List<ChatPagingResponseDto> chatMessageDtoList, Long workSpaceId,
			String cursor) {
		...
		Slice<Chat> chatSlice = chatRepository.findAllByCreatedAtBeforeAndWorkSpace_IdOrderByCreatedAtDesc(lastCursor,
				workSpaceId, PageRequest.of(0, 30));
